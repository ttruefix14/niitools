...
class Params:
    ...
    def __init__(self, params):
        ...
        rename_cols = (
            [i.split("=") for i in params[1].values] if params[1].value else None
        )
...
class Errors:
    ...
    columns = [
        ...
        "Ошибка",
        "Значение",
        "Тип поля (обязательное, условное, необязательное)",
    ...
    def __init__(self, p10):
        ...
        for i in zip(
            ...
            self.p10[i[0]][i[1]] = [
                ...
                json.loads(i[5]) if i[5] is not None else None,
                i[4],
            ]
        ...
        if r_name not in self.p10.keys():
            self.append(
                [
                    ...
                    None,
                    None,
                    "Название слоя не соответствует десятому приказу",
                    None,
                    None,
                    None,
                ]
        ...
        if cid not in self.p10[r_name]["CLASSID"][2]:
            self.errors.append(
                [
                    ...
                    "CLASSID",
                    None,
                    "Код объекта не соответствует десятому приказу",
                    cid,
                    "О",
                    None,
                ]
    ...
    def good_str(self, string):
        ...
        bad = re.findall(r"[&\n\t\r\'<>\u0008\x02]+", string)
        bad = re.findall(
            r"[\x00-\x08\x0b\x0c\x0e-\x1F\uD800-\uDFFF\uFFFE\uFFFF]+", string
        )
        if bad:
            return (
                False,
                "Недопустимые символы в строке (Знак табуляции \\t, знак абзаца \\n, знак возврата каретки \\r, одинарные кавычки ', знаки < >, &, x08(бекспейс в юникоде))",
                bad,
            )
        else:
            ...
            if col_required == "Н" and str_check[1] == "Пустая строка":
                ...
                self.append(
                    [
                        ...
                        None,
                        str_check[1],
                        repr(string)[1:-1] if string else None,
                        col_required,
                        str_check[2],
                    ]
                )
...
def tab_to_df(in_table, input_fields=None, where_clause=None):
    ...
    if input_fields:
        ...
        final_fields = [
            field.name.upper() if field.name != shapeFieldName else "SHAPE@"
...
def execute():
    ...
    for layer in layers:
        ...
        for i, row in df.iterrows():
            ...
            if "CLASSID" in df.columns:
                errors.check_classid(row["CLASSID"], i, r_name, b_name)

            if "GLOBALID" in df.columns:
                errors.is_correct_uuid(row["GLOBALID"], i, r_name, b_name)
            ...
            for col in check_columns:
                ...
                if col == "OKTMO":
                    errors.check_oktmo(row["OKTMO"], i, r_name, b_name)
                    continue
                ...
                if col in ["DATE_START", "DATE_CLOSE"]:
                    errors.is_date(
                        row[col], i, col, col_required, r_name, b_name
                    )
                    continue
                ...
                if not errors.check_condition(r_name, col, row):
                    continue
                else:
                    pass
                    ...
                    if row[col] in domain:
                        ...
                        errors.append(
                            [
                                ...
                                i,
                                col,
                                errors.p10[r_name][col][4],
                                "Значение не входит в словарь",
                                row[col],
                                col_required,
                                None,
                            ]
                ...
                if col_type == "Целое":
                    if row[col] == None or row[col] == 0:
                        errors.append(
                            [
                                ...
                                col,
                                None,
                                "Поле должно быть заполнено целым числом",
                                row[col],
                                col_required,
                                None,
                            ]
                    ...
                    if row[col] == None or row[col] == 0:
                        errors.append(
                            [
                                ...
                                col,
                                None,
                                "Поле должно быть заполнено вещественным числом",
                                row[col],
                                col_required,
                                None,
                            ]
                    ...
                    errors.append(
                        [
                            ...
                            col,
                            None,
                            f"Неизвестный тип поля {col_type}",
                            row[col],
                            col_required,
                            None,
                        ]
    ...
    gid_df.to_excel(
        writer, engine="openpyxl", sheet_name="Повторные GLOBALID", index=False
    )
