...
def geom_to_gml(geom, clipping_mask, no_clip):
    ...
    return ogr_shp.ExportToGML(options=["FORMAT=GML3", 'NAMESPACE_DECL=YES', 'GMLID=change']) # 'GML3_LINESTRING_ELEMENT=curve'
...
def create_gml(xsd):
    et.register_namespace("", "http://www.opengis.net/gml")
    ...
    envelope = '<gml:boundedBy  xmlns:gml="http://www.opengis.net/gml"><gml:Envelope srsName="EPSG:3857"><gml:lowerCorner>{} {}</gml:lowerCorner><gml:upperCorner>{} {}</gml:upperCorner></gml:Envelope></gml:boundedBy>' #.format(round(extent.XMin, 2), round(extent.YMin, 2), round(extent.XMax, 2), round(extent.YMax, 2))
...
def save_gml(gml, dirname, filename, p10):
    ...
    for elem in gml.iter():
        ...
        if 'gml:id' in elem.attrib:
            elem.attrib['gml:id'] = name + '.' + str(next(layers_ids[name]))
        if '{http://www.opengis.net/gml}id' in elem.attrib:
            elem.attrib['{http://www.opengis.net/gml}id'] = name + '.' + str(next(layers_ids[name]))
