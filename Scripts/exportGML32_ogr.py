...
class Params:
    ...
    def __init__(self, params):
        ...
        rename_cols = (
            [i.split("=") for i in params[3].values] if params[3].values else None
        )
        ...
        self.OKTMO = params[
            6
        ].valueAsText  # Введите ОКТМО поселения, если где-то октмо будет не заполнено - оно заполнится этим автоматически
        self.empty_value = params[7].value

        self.ignore_cid = params[8].values

        self.omz_definition = (
            [i.split("<>") for i in params[9].values] if params[9].value else None
        )
...
class P10:
        ...
        for i in zip(
            ...
            self.p10[i[0]][i[1]] = [
                ...
                (
                    [
                        d
                        for d in dom_table["Code"]
                        .loc[dom_table["Domain"] == i[4]]
                        .to_list()
                    ]
                    if i[4]
                    else (
                        [
                            c
                            for c in cid_table["CLASSID"]
                            .loc[cid_table["Layer"] == i[0]]
                            .to_list()
                        ]
                        if i[1] == "CLASSID"
                        else None
    ...
    def check_columns(self, df, r_name, b_name, rename_cols):
        ...
        check_columns = sorted(
            list(check_columns), key=lambda x: required_columns_list.index(x)
        )
...
def tab_to_gdf(
    ...
    if input_fields:
        ...
        final_fields = [
            field.name.upper() if field.name != shapeFieldName else "SHAPE@WKT"
            for field in arcpy.Describe(in_table).fields
        ]
    data = [
        row
    ...
    fc_dataframe = gpd.GeoDataFrame(
        df, geometry=gs, crs=srs.factoryCode
    )  # srs.exportToString()
...
def create_custom_transformation(inputSRS, outputSRS, customGeoTransfm, geoTransfm):
    ...
    if (
        inputSRS == outputSRS
        or inputSRS.exportToString() == outputSRS.exportToString()
        or inputSRS.factoryCode == outputSRS.factoryCode
    ):
        return None
    if inputSRS.factoryCode == 4326:
        return None
    dirname = os.path.join(
        os.getenv("APPDATA"), "Esri", "ArcGISPro", "ArcToolbox", "CustomTransformations"
    )
    ...
    arcpy.management.CreateCustomGeoTransformation(
        geotransfName.format(n), inputSRS, outputSRS, customGeoTransfm
    )
    ...
    for col, value in row_object.items():
        ...
        if p10.get(name).get(col)[1] == "Целое":
            try:
                ...
                if (
                    p10.get(name).get(col)[4] is not None
                ):  # если домен то None если не домен то 0?? фига знает как лучше
                    value = 0
...
def create_gml(dirname, filename, xsd, gml_version):
    ...
    outDataSource = driver.Create(
        path, 0, 0, 0, 0, [f"FORMAT={gml_version}", "WRITE_FEATURE_BOUNDED_BY=NO"]
    )  # , 'XSISCHEMA=OFF' {xsd} fgistp-10-izm-698.xsd fr'XSISCHEMAURI=http://fgistp {xsd}', r'TARGET_NAMESPACE=http://fgistp' 'PREFIX=fgistp', fr'TARGET_NAMESPACE=http://www.opengis.net/gml{r"/3.2" if gml_version == "GML3.2" else ""}',  f'GML_FEATURE_COLLECTION={"YES" if gml_version == "GML3" else "NO"}'
    return outDataSource
...
def fc_to_gml(outSource, layerName, gdf, epsg, mask, oktmo, p10):
    ...
    gdf.geometry = gdf.apply(
        lambda row: removeLowerDimension(row.geometry, gtype), axis=1
    )
    gdf = gdf.loc[~gdf.geometry.isnull()]
    ...
    invalid = [
        {
            "guid": gid,
            "layerName": layerName,
        } for gid, geom in zip(gdf.GLOBALID, gdf.geometry) if not shapely.is_valid(geom)
    ]
    if len(invalid) > 0:
        not_valid_geoms += invalid
...
def splitXml(path, splitSize, fileSize):
    ...
    blank = [
        xmlstr[: regex.search(r"</.*boundedBy>\n*\t*", xmlstr).end()],
        regex.search(r"(</.*FeatureCollection>)", xmlstr).groups()[0],
    ]

    blank_size = len(blank[0].encode("utf-8")) + len(blank[1].encode("utf-8"))

    current = xmlstr[
        regex.search(r"</.*boundedBy>\n*\t*", xmlstr)
        .end() : regex.search(r"(</.*FeatureCollection>)", xmlstr)
        .start()
    ].encode("utf-8")
...
def execute():
    ...
    border_loc = (
        p10["AdmBorder"]["CLASSID"][2]
        + p10["AdmeNP"]["CLASSID"][2]
        + p10["AdmeMO"]["CLASSID"][2]
    )
    ...
    geotransfName = create_custom_transformation(
        inputSRS, outputSRS, params.customGeoTransfm, params.geoTransfm
    )
    if params.mask_layer:
        clipping_mask = shapely.from_wkt(
            [
                row[0]
                for row in arcpy.da.SearchCursor(
                    params.mask_layer,
                    ["SHAPE@WKT"],
                    spatial_reference=outputSRS,
                    datum_transformation=geotransfName,
                )
            ][0]
        )
    else:
    ...
    border_gml = create_gml(
        params.output_dirname,
    ...
    omz_gml = create_gml(
        params.output_dirname,
        "Карта планируемого размещения объектов.gml",
        params.xsd[0],
        params.gml_version,
    )
    fz_gml = create_gml(
    ...
    mo_gml = create_gml(
        params.output_dirname,
        "Материалы по обоснованию в виде карт.gml",
        params.xsd[0],
        params.gml_version,
    )
    ...
    for lyr in layers:
        ...
        table, epsg = tab_to_gdf(
            lyr, spatial_reference=outputSRS, datum_transformation=geotransfName
        )
        ...
        fc_to_gml(
            ...
            border_table,
            epsg,
            clipping_mask if not no_clip else None,
            params.OKTMO,
            p10.p10,
        )
        ...
        if "Карта планируемого размещения объектов" in filename:
            if filename == "Карта планируемого размещения объектов.gml":
                shutil.copy(
                    ...
                    os.path.join(
                        params.output_dirname,
                        "Приложение к положению о территориальном планировании в форме электронного документа.xml",
                    ),
                )
            else:
                shutil.copy(
                    os.path.join(params.output_dirname, filename),
                    os.path.join(
                        params.output_dirname,
                        "Приложение к положению о территориальном планировании в форме электронного документа_"
                        + re.search(r"_(.*)\.", filename).groups(0)[0]
                        + ".xml",
                    ),
                )
        ...
        if "Материалы по обоснованию в виде карт" in filename:
            if filename == "Материалы по обоснованию в виде карт.gml":
                shutil.copy(
                    ...
                    os.path.join(
                        params.output_dirname,
                        "Материалы по обоснованию в формате xml.xml",
                    ),
                )
            else:
                shutil.copy(
                    ...
                    os.path.join(
                        params.output_dirname,
                        "Материалы по обоснованию в формате xml_"
                        + re.search(r"_(.*)\.", filename).groups(0)[0]
                        + ".xml",
                    ),
                )
